  ...
  return (
    <div className="p-3.5 sm:p-6 max-w-[1200px] mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6">
        ...
        <div className="md:col-span-5 bg-white rounded-xl border border-[#E8E4DC] p-4 sm:p-5 space-y-4 sm:space-y-5">
          ...
          <div className="space-y-3 text-xs">
            <div className="p-3 bg-[#F7F4EC] rounded-xl border border-[#E8E4DC] space-y-1.5">
              ...
              <p className="font-mono font-bold text-[#0B132B] flex items-center gap-1.5 break-all">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-1 border-b border-[#F1EDE4] gap-2 flex-wrap sm:flex-nowrap">
                <span className="text-[#64748B]">Firebase Project</span>
                <span className="font-mono font-bold text-[#0B132B] break-all">arklintech-5063d</span>
              </div>
              <div className="flex items-center justify-between py-1 border-b border-[#F1EDE4] gap-2 flex-wrap sm:flex-nowrap">
              ...
              <div className="flex items-center justify-between py-1 border-b border-[#F1EDE4] gap-2 flex-wrap sm:flex-nowrap">
                ...
                <span className="font-mono font-bold text-[#1463FF] break-all">Firebase ID Token (Bearer)</span>
              </div>
              <div className="flex items-center justify-between py-1 gap-2 flex-wrap sm:flex-nowrap">
                <span className="text-[#64748B]">Public Contact Mailbox</span>
                <span className="font-mono font-bold text-[#0B132B] break-all">work@arklintech.com</span>
              </div>
            </div>
        ...
        <div className="md:col-span-7 bg-white rounded-xl border border-[#E8E4DC] overflow-hidden flex flex-col">
          ...
          <div className="divide-y divide-[#F1EDE4] flex-1">
            {securityPolicies.map((p, idx) => (
              <div key={idx} className="p-3.5 sm:p-4 flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4 hover:bg-[#FDFBF7] transition-colors">
                <div>
                  <p className="font-bold text-xs text-[#0B132B]">{p.name}</p>
                  <p className="text-[11px] text-[#64748B] mt-0.5">{p.desc}</p>
                </div>
                <span className="font-mono text-[9px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full self-start shrink-0">
                  {p.status}
                </span>
              </div>
            ))}
      ...
      <div className="bg-white rounded-xl border border-[#E8E4DC] overflow-hidden">
        ...
        <div className="divide-y divide-[#F1EDE4]">
          {auditEvents.map((log, idx) => (
            <div key={idx} className="px-4 sm:px-5 py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 text-xs">
              <div className="space-y-0.5">
                <p className="font-semibold text-[#0B132B]">{log.event}</p>
                <p className="font-mono text-[10px] text-[#64748B] break-all">{log.user}</p>
              </div>
              <div className="sm:text-right flex items-center sm:block gap-2">
                ...
                <p className="font-mono text-[10px] text-[#94A3B8] sm:mt-0.5">{log.ts}</p>
              </div>
            </div>
          ))}
        </div>
