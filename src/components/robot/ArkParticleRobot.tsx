...
export default function ArkParticleRobot({
  ...
  useEffect(() => {
    ...
    // Scroll to top on mount so particle robot is always visible on reload
    ...
    let particleCount = 0;
    let p_nx: Float32Array = new Float32Array(0);
    let p_ny: Float32Array = new Float32Array(0);
    let p_dispX: Float32Array = new Float32Array(0);
    let p_dispY: Float32Array = new Float32Array(0);
    let p_cohesion: Float32Array = new Float32Array(0);
    let p_depth: Float32Array = new Float32Array(0);
    let p_seed: Float32Array = new Float32Array(0);
    let p_size: Uint8Array = new Uint8Array(0);
    let p_r: Uint8Array = new Uint8Array(0);
    let p_g: Uint8Array = new Uint8Array(0);
    let p_b: Uint8Array = new Uint8Array(0);
      ...
      img.onload = () => {
        ...
        for (let y = 0; y < srcH; y += step) {
          for (let x = 0; x < srcW; x += step) {
            ...
            if (isLogo) {
              cohesion = 0.86 + seed * 0.08; depth = 0.8;
            } else if (dEyeL < 0.08 || dEyeR < 0.08) {
              cohesion = 0.88 + seed * 0.08; depth = 1.45;
            } else if (dCrest < 0.09) {
              cohesion = 0.84 + seed * 0.08; depth = 1.2;
            } else if (dMouth < 0.09) {
              cohesion = 0.76 + seed * 0.08; depth = 1.1;
            } else if (Math.hypot(nx - 0.5, ny - 0.46) < 0.28) {
              cohesion = 0.60 + seed * 0.15; depth = 1.0;
            } else if (nx < 0.18 || nx > 0.82 || ny < 0.10) {
              cohesion = 0.18 + seed * 0.15; depth = 0.6;
            } else {
              cohesion = 0.38 + seed * 0.20; depth = 0.9;
            }

            const angle = isLogo
              ? Math.PI * 0.5 + (nx - 0.5) * 0.8 + (seed - 0.5) * 0.6
              : Math.atan2(ny - 0.44, nx - 0.50) + (seed - 0.5) * 1.4;
        ...
        for (let i = 0; i < particleCount; i++) {
          const pt = rawList[i];
          p_nx[i] = pt.nx; p_ny[i] = pt.ny;
          p_dispX[i] = pt.dispX; p_dispY[i] = pt.dispY;
          p_cohesion[i] = pt.cohesion; p_depth[i] = pt.depth;
          p_seed[i] = pt.seed; p_size[i] = pt.size;
          p_r[i] = pt.r; p_g[i] = pt.g; p_b[i] = pt.b;
        }
    ...
    // ── 60 FPS Render Tick ────────────────────────────────────────────────
    // Named as `tick` — completely separate from any closures above.
    ...
    const tick = (t: number) => {
      ...
      if (Math.abs(targetDissolve - dissolve) < 0.0001) dissolve = targetDissolve;
      ...
      if (isMobile) {
        ...
        if (renderH > maxMobileH) { renderH = maxMobileH; renderW = renderH * imgAspect; }
        ox = (width - renderW) * 0.5;
        oy = (height - renderH) * 0.48;
      } else {
        if (canvasAspect > imgAspect) {
          renderH = height * 0.94; renderW = renderH * imgAspect;
        } else {
          renderW = width * 0.96; renderH = renderW / imgAspect;
        }
        ox = (width - renderW) * 0.5;
      ...
      const sinT1 = Math.sin(time * 1.37);
      const cosT1 = Math.cos(time * 1.11);
      const sinT2 = Math.sin(time * 0.80);
      const cosT2 = Math.cos(time * 0.70);
      ...
      // Pause loop when fully dissolved — resume via onScroll
  ...
  return (
    <div
      ...
      style={{ width: '100%', height: '350vh', backgroundColor: '#000000' }}
