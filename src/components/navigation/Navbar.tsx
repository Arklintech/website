  ...
  return (
    <>
      ...
      >
        <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 flex items-center justify-between gap-4">
          ...
          {/* =========================================
              ...
              ========================================= */}
          <nav className="hidden lg:flex items-center gap-4 xl:gap-7 bg-z-surface/80 border border-z-border/80 px-6 py-2.5 rounded-full backdrop-blur-md shadow-[0_4px_25px_rgba(0,0,0,0.4)]">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.id}
                href={item.href}
                className="font-mono text-[11px] xl:text-xs uppercase tracking-[0.12em] text-z-muted hover:text-z-white whitespace-nowrap transition-colors duration-200 py-0.5"
              >
                {item.label}
              </a>
            ))}
          ...
          <div className="hidden sm:flex items-center gap-3 xl:gap-4 shrink-0">
            ...
            <button
              ...
              className="z-btn-primary group text-xs py-2.5 px-5 whitespace-nowrap"
            >
          ...
          <div className="flex lg:hidden items-center gap-2">
            <button
              onClick={onOpenProjectModal}
              className="sm:hidden px-3 py-1.5 rounded bg-z-blue-500 text-z-white font-mono text-[10px] uppercase font-semibold flex items-center gap-1 shrink-0"
            >
              <span>START</span>
            ...
            <button
              ...
              className="p-2 rounded border border-z-border bg-z-surface text-z-text hover:text-z-white shrink-0"
            >
      ...
      {/* =========================================
          ...
          ========================================= */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-z-black/95 backdrop-blur-xl pt-24 px-6 flex flex-col justify-between pb-8 lg:hidden animate-in fade-in duration-200">
          <div className="flex flex-col space-y-5">
            ...
            {NAV_ITEMS.map((item) => (
              <a
                ...
                className="font-display text-lg tracking-wider text-z-text hover:text-z-blue-400 py-1 transition-colors border-b border-z-border/30"
              >
                {item.label}
              </a>
            ))}
          ...
          <div className="space-y-4 pt-6 border-t border-z-border">
            <div className="flex items-center justify-between text-xs font-mono text-z-muted bg-z-surface-2 p-3 rounded border border-z-border">
              <span className="flex items-center gap-2">
                <span className="status-indicator" /> SYSTEM STATE
              </span>
            ...
            <button
              ...
              className="w-full z-btn-primary justify-center py-3 text-xs"
            >
              <span>START A PROJECT</span>
